...
export default function App() {
  ...
  const [youtubeExportCount, setYoutubeExportCount] = useState(1);
  ...
  const downloadTranscription = () => {
    ...
    if (inputType === 'youtube') {
      a.download = `Youtube Transcribe ${youtubeExportCount}.txt`;
      setYoutubeExportCount(prev => prev + 1);
    } else {
    ...
    <div className="min-h-screen bg-[#fafafa] text-zinc-900 font-sans selection:bg-zinc-200">
      ...
      <nav className="fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-xl border-b border-zinc-200 z-50 flex items-center justify-between px-6">
        <div className="flex items-center gap-2.5">
          <img src="/Lexis.png" alt="Lexis Logo" className="w-8 h-8 object-contain rounded-lg shadow-sm bg-zinc-900" />
          <span className="font-bold tracking-tight text-lg">Lexis</span>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setView(view === 'converter' ? 'transcriber' : 'converter')}
            className="px-4 py-1.5 text-xs font-bold bg-zinc-900 text-white rounded-full hover:bg-zinc-800 transition-colors shadow-sm"
          >
            {view === 'converter' ? 'Go to Transcriber' : 'Mp3 Converter'}
          </button>
        </div>
      </nav>
            ...
            <aside className="lg:sticky lg:top-32 space-y-6">
              ...
              <div className="bg-zinc-900 rounded-3xl p-6 text-white space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-2xl font-light">Gemini<span className="text-xs text-zinc-500 ml-0.5 font-medium">3 FLASH</span></p>
      ...
      <footer className="w-full py-12 flex items-center justify-center px-6">
        <div className="bg-zinc-900 border border-zinc-700 px-5 py-2 rounded-full text-[11px] font-semibold uppercase tracking-widest text-zinc-300 shadow-lg shadow-zinc-900/20 flex items-center gap-2">
